...
type RestOf<T extends IDossier | NewDossier> = Omit<T, 'dateSoumission'> & {
...
export class DossierService {
  ...
  protected convertDateFromClient<T extends IDossier | NewDossier | PartialUpdateDossier>(dossier: T): RestOf<T> {
    return {
      ...dossier,
      dateSoumission: dossier.dateSoumission?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restDossier: RestDossier): IDossier {
    return {
      ...restDossier,
      dateSoumission: restDossier.dateSoumission ? dayjs(restDossier.dateSoumission) : undefined,
    };
  }
