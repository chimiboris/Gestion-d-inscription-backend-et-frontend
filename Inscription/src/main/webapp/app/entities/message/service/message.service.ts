...
type RestOf<T extends IMessage | NewMessage> = Omit<T, 'dateEnvoie'> & {
...
export class MessageService {
  ...
  protected convertDateFromClient<T extends IMessage | NewMessage | PartialUpdateMessage>(message: T): RestOf<T> {
    return {
      ...message,
      dateEnvoie: message.dateEnvoie?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restMessage: RestMessage): IMessage {
    return {
      ...restMessage,
      dateEnvoie: restMessage.dateEnvoie ? dayjs(restMessage.dateEnvoie) : undefined,
    };
  }
